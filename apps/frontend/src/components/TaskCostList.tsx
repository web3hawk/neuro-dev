...
function TaskCostList() {
  ...
  // Calculate total estimated cost for filtered tasks
  const totalEstimatedCost = filteredTasks.reduce((sum, task) => sum + (task.estimated_cost || 0), 0);
  ...
  const columns = [
    ...
    },
    {
      ...
      sorter: (a: TaskItem, b: TaskItem) => (a.estimated_cost || 0) - (b.estimated_cost || 0),
    },
    {
      title: '预计天数',
    ...
    },
    {
      ...
      render: (record: TaskItem) => {
        if (record.estimated_cost && record.estimated_days) {
          const dailyCost = record.estimated_cost / record.estimated_days;
          return <Tag color="purple">¥{dailyCost.toLocaleString()}/天</Tag>;
        }
        return <Text type="secondary">无法计算</Text>;
      },
    },
    {
      title: '创建时间',
    ...
    <div>
      <Card>
        <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <Title level={4}>任务成本管理</Title>
            <Text type="secondary">
              查看和分析所有项目的任务成本情况
            </Text>
            <div style={{ marginTop: 8 }}>
              <Text strong style={{ color: '#52c41a', fontSize: '16px' }}>
                总预计成本: ¥{totalEstimatedCost.toLocaleString()}
              </Text>
          ...
          <div>
            <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/tasks/create?expense_type=cost')}>
              新增成本
            </Button>
          </div>
        </div>
