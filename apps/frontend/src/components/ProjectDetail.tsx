...
function ProjectDetail() {
  ...
  const loadProject = async () => {
    try {
      ...
      if ((response as any).data?.success) {
        setProject((response as any).data.data as ProjectItem);
      } else {
        message.error('项目不存在');
        navigate('/projects');
      }
  ...
  const handleTaskUpdate = (stats: any) => {
    setTaskStats(stats);
  };
  ...
  const loadLogs = async () => {
    try {
      ...
      if ((response as any).data?.success) {
        setLogs((response as any).data.data.logs);
      }
    } catch (error) {
  ...
  const loadFiles = async () => {
    try {
      ...
      if ((response as any).data?.success) {
        setFiles((response as any).data.data.files);
      }
    } catch (error) {
  ...
  const handleDownloadProject = async () => {
    try {
      const response = await api.post(`/api/projects/${id}/download`);
      if ((response as any).data?.success) {
        message.success('项目导出成功!');
        window.open((response as any).data.data.download_url);
      }
    } catch (error) {
      console.error('Failed to start project:', error);
      ...
      <Card>
        <Tabs activeKey={activeTab} onChange={setActiveTab}>
          ...
          <TabPane tab="执行日志" key="logs">
            <List
              dataSource={logs}
              renderItem={(log: string, index: number) => (
                <List.Item>
          ...
          <TabPane tab="生成文件" key="files">
            <List
              ...
              renderItem={(file: any) => (
                <List.Item
                  actions={[
                    <Button key="view" icon={<EyeOutlined />} size="small">查看</Button>,
                    <Button key="dl" icon={<DownloadOutlined />} size="small">下载</Button>
                  ]}
                >
                  <List.Item.Meta
                    avatar={<FileOutlined />}
