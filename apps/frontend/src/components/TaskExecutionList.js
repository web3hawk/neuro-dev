...
function TaskExecutionList() {
        ...
        for (const project of projectList) {
          try {
            ...
            if (tasksResponse.data.success && tasksResponse.data.data) {
              const projectTasks = tasksResponse.data.data.map(task => ({
                ...task,
  ...
  const applyFilters = () => {
    ...
    // Filter by completion date range (only for completed tasks)
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'assigned_role',
      render: (role) => role ? <Tag color="purple">{role}</Tag> : <Text type="secondary">未分配</Text>,
    },
    {
      title: '创建时间',
          ...
          <Row gutter={[16, 16]}>
            ...
            <Col span={12}>
              <Space direction="vertical" style={{ width: '100%' }}>
                ...
                <RangePicker
                  value={filters.completionDateRange}
                  onChange={(dates) => handleFilterChange('completionDateRange', dates)}
                  style={{ width: '100%' }}
                  placeholder={['开始时间', '结束时间']}
                />
              </Space>
